...
const DragAndDropWrapper: React.FC<{
	...
	const handleDrop = async (event: React.DragEvent<HTMLDivElement>) => {
		...
		const filePathsFromEditors = editorData
			...
			: [];
		const allFilePaths = [
			...new Set([...filePathsFromFiles, ...filePathsFromEditors]),
		];
	...
	return (
		...
		>
			{isDragOver && (
				<div className="absolute inset-0 z-10 opacity-75 bg-void-bg-1 rounded-md" />
			)}
			{children}
		</div>
	);
...
export const VoidChatArea: React.FC<VoidChatAreaProps> = ({
	...
	return setSelections && showSelections ? (
		<DragAndDropWrapper>{content}</DragAndDropWrapper>
	) : (
		content
	);
};
...
export const ButtonSubmit = ({
	...
	return (
		<button
			type="button"
			className={`rounded-sm flex-shrink-0 flex-grow-0 flex items-center justify-center
...
const UserMessageComponent = ({
	...
	if (mode === "display") {
		// File references are now inline in displayContent (e.g., "@filename.ext")
		// so we don't need to show SelectedFiles badges separately
		chatbubbleContents = (
		...
		>
            ...
            ${
							mode === "edit"
								? ""
								: mode === "display"
									? "p-2 flex flex-col bg-void-bg-1 text-void-fg-1 overflow-x-auto cursor-pointer border border-void-border-3 hover:border-void-border-2"
									: ""
						}
        `}
				onClick={() => {
...
export const ListableToolItem = ({
	...
	return (
		...
		>
			...
			<div
				className={`${isSmall ? "italic text-void-fg-4 flex items-center" : ""}`}
			>
				{name}
			</div>
		</div>
...
const CommandTool = ({
	...
	if (toolMessage.type === "success") {
		...
		let msg: string;
		if (type === "run_command")
			msg = toolsService.stringOfResult["run_command"](
				toolMessage.params,
				result,
			);
		else
			msg = toolsService.stringOfResult["run_persistent_command"](
				toolMessage.params,
				result,
			);

		if (type === "run_persistent_command") {
			componentParams.info = persistentTerminalNameOfId(
				toolMessage.params.persistentTerminalId,
			);
		}
			...
			return (
				<>
					...
					{chatMessage.type === "tool_request" ? (
						<div
							className={`${isCheckpointGhost ? "opacity-50 pointer-events-none" : ""}`}
						>
