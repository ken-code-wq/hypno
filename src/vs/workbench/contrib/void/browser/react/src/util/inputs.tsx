...
// type guard
const isConstructor = (f: any): f is { new (...params: any[]): any } => {
	return !!f.prototype && f.prototype.constructor === f;
};

export const WidgetComponent = <CtorParams extends any[], Instance>({
	ctor,
	propsFn,
	dispose,
	onCreateInstance,
	...
	ctor:
		| { new (...params: CtorParams): Instance }
		| ((container: HTMLDivElement) => Instance);
	...
	useEffect(() => {
		const instance = isConstructor(ctor)
			? new ctor(...propsFn(containerRef.current!))
			: ctor(containerRef.current!);
	...
	return (
		<div
			ref={containerRef}
			className={className === undefined ? `w-full` : className}
		>
			{children}
		</div>
	);
...
export const VoidSimpleInputBox = ({
	value,
	onChangeValue,
	placeholder,
	className,
	...
	return (
		...
		"
			...
			propsFn={useCallback(
				...
				[contextViewProvider, placeholder, multiline],
			)}
			...
			onCreateInstance={useCallback(
				(instance: InputBox) => {
					...
					if (inputBoxRef) inputBoxRef.current = instance;
...
export const VoidSlider = ({
	...
	return (
		<div className={`inline-flex items-center flex-shrink-0 ${className}`}>
			...
			<div
				className={`relative flex-shrink-0 ${disabled ? "opacity-25" : ""}`}
				style={{
					width,
	...
	return (
		<div className={`inline-block relative ${className}`}>
			...
			{isOpen && (
				<div
					ref={refs.setFloating}
					className="z-[100] bg-void-bg-1 border-void-border-3 border rounded-sm shadow-lg"
					style={{
						position: strategy,
						top: y ?? 0,
						left: x ?? 0,
				...
				>
					<div className="overflow-auto max-h-80">
						{options.map((option) => {
							const thisOptionIsSelected = getOptionsEqual(
								option,
								selectedOption,
							);
							const optionName = getOptionDropdownName(option);
		...
		`}
			...
			onCreateInstance={useCallback(
				(instance: SelectBox) => {
					...
					if (selectBoxRef) selectBoxRef.current = instance;
